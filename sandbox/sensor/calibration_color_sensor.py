...
class CalibColor:  # TODO: include automatic
    ...
    def __init__(self,  calibprojector: str = None, name: str = 'kinectv2', **kwargs):
        ...
        self.frame_raw = self.sensor.get_color()
    ...
    def _refresh_panel_frame(self):
        ...
        self.fig_frame = self.projector.ax.imshow(self.sensor.get_color())
        print(self.sensor.get_color().shape)
    ...
    def update_notebook_frame(self):
        ...
        rec_t = plt.Rectangle((0, self.sensor.c_height - self.sensor.c_top), self.sensor.c_width, self.sensor.c_top,
                              fc=self.c_margin, alpha=self.margin_alpha)
        rec_r = plt.Rectangle((self.sensor.c_width - self.sensor.c_right, 0), self.sensor.c_right, self.sensor.c_height,
                              fc=self.c_margin, alpha=self.margin_alpha)
        rec_b = plt.Rectangle((0, 0), self.sensor.c_width, self.sensor.c_bottom,
                              fc=self.c_margin, alpha=self.margin_alpha)
        rec_l = plt.Rectangle((0, 0), self.sensor.c_left, self.sensor.c_height,
                              fc=self.c_margin, alpha=self.margin_alpha)
    ...
    def calibrate_sensor(self):
        widgets = pn.WidgetBox('<b>Load a projector calibration file</b>',
                               ...
                               self._widget_c_top,
                               self._widget_c_right,
                               self._widget_c_bottom,
                               self._widget_c_left,
    ...
    def _create_widgets(self):
        ...
        self._widget_c_top = pn.widgets.IntSlider(name='Sensor top margin',
                                                  bar_color=self.c_margin,
                                                  value=self.sensor.c_top,
                                                  start=1,
                                                  end=self.sensor.c_height)
        self._widget_c_top.param.watch(self._callback_c_top, 'value', onlychanged=False)

        self._widget_c_right = pn.widgets.IntSlider(name='Sensor right margin',
                                                    bar_color=self.c_margin,
                                                    value=self.sensor.c_right,
                                                    start=1,
                                                    end=self.sensor.c_width)
        self._widget_c_right.param.watch(self._callback_c_right, 'value', onlychanged=False)

        self._widget_c_bottom = pn.widgets.IntSlider(name='Sensor bottom margin',
                                                     bar_color=self.c_margin,
                                                     value=self.sensor.c_bottom,
                                                     start=1,
                                                     end=self.sensor.c_height)
        self._widget_c_bottom.param.watch(self._callback_c_bottom, 'value', onlychanged=False)

        self._widget_c_left = pn.widgets.IntSlider(name='Sensor left margin',
                                                   bar_color=self.c_margin,
                                                   value=self.sensor.c_left,
                                                   start=1,
                                                   end=self.sensor.c_width)
        self._widget_c_left.param.watch(self._callback_c_left, 'value', onlychanged=False)

        self._widget_c_min = pn.widgets.IntSlider(name='Vertical minimum',
                                                  bar_color=self.c_under,
                                                  value=self.sensor.s_min,
                                                  start=0,
                                                  end=2000)
        self._widget_c_min.param.watch(self._callback_c_min, 'value', onlychanged=False)

        self._widget_c_max = pn.widgets.IntSlider(name='Vertical maximum',
                                                  bar_color=self.c_over,
                                                  value=self.sensor.s_max,
                                                  start=0,
                                                  end=2000)
        self._widget_c_max.param.watch(self._callback_c_max, 'value', onlychanged=False)
        ...
        self._widget_json_filename.value = _calibration_dir + 'my_color_calibration.json'
        ...
        # sensor callbacks
    def _callback_c_top(self, event):
        self.sensor.c_top = event.new
        # change plot and trigger panel update
        self.update_notebook_frame()

    def _callback_c_right(self, event):
        self.sensor.c_right = event.new
        # self._refresh_panel_frame() #TODO: dirty workaround
        self.update_notebook_frame()

    def _callback_c_bottom(self, event):
        self.sensor.c_bottom = event.new
        self.update_notebook_frame()

    def _callback_c_left(self, event):
        self.sensor.c_left = event.new
        # self._refresh_panel_frame()  # TODO: dirty workaround
        self.update_notebook_frame()

    def _callback_c_min(self, event):
        self.sensor.s_min = event.new
        # self._refresh_panel_frame()  # TODO: dirty workaround
        self.update_notebook_frame()

    def _callback_c_max(self, event):
    ...
    def _callback_json_save(self, event):
        if self.sensor.json_filename is not None:
            self.sensor.save_color_json(file=self.sensor.json_filename)
